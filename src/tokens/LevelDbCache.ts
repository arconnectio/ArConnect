...
/**
 * Source from https://github.com/warp-contracts/warp/blob/f901cb51f07f4e1d7e21c372520fe80ad2c8ed34/src/cache/impl/LevelDbCache.ts
 * Credit to the Warp / Redstone Team
 */
 ...
 */
// @ts-expect-error
export default class LevelDbCache<V = any> implements SortKeyCache<V> {
  ...
  constructor(cacheOptions: CacheOptions) {
    if (cacheOptions.inMemory) {
      ...
      if (!cacheOptions.dbLocation) {
        throw new Error(
          "LevelDb cache configuration error - no db location specified"
        );
      }
  ...
  async get(
    ...
    const contractCache = this.db.sublevel<string, any>(contractTxId, {
      valueEncoding: "json"
    });
  ...
  async getLessOrEqual(
    ...
    // manually opening to fix https://github.com/Level/level/issues/221
    await contractCache.open();
    const keys = await contractCache
      .keys({ reverse: true, lte: sortKey, limit: 1 })
      .all();
    if (keys.length) {
      return {
        sortKey: keys[0],
  ...
  async put(stateCacheKey: CacheKey, value: V): Promise<void> {
    const contractCache = this.db.sublevel<string, any>(
      stateCacheKey.contractTxId,
      { valueEncoding: "json" }
    );
    // manually opening to fix https://github.com/Level/level/issues/221
    await contractCache.open();
