...
export default function Vaults() {
  ...
  // router
  const [matches, params] = useRoute<{ address?: string }>("/vaults/:address?");
  ...
  useEffect(() => {
    ...
    setLocation("/vaults/" + firstWallet.address);
    ...
    <>
      <Wrapper>
        <SearchWrapper>
          ...
          />
          <AddWalletButton onClick={() => setLocation("/vaults/new")}>
          ...
          >
            {wallets.filter(filterSearchResults).map((wallet) => (
              <WalletListItem
                ...
                onClick={() => setLocation("/vaults/" + wallet.address)}
                key={wallet.address}
              />
            ))}
          </Reorder.Group>
